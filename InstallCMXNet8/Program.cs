...
namespace InstallCMX
{
    public class Program
    {
        ...
        private void RealMain( string[] args )
        {
            ...
            Sup = new Support();
            Sup.LogDebugMessage( $"CMX multiplatform installer {Support.Version()} - {Support.Copyright()}" );
            Console.WriteLine( $"CMX multiplatform installer {Support.Version()} - {Support.Copyright()}" );
                ...
                using ( ZipArchive archive = ZipFile.OpenRead( ArchiveToInstall ) )
                {
                    foreach ( ZipArchiveEntry entry in archive.Entries )
                    {
                        ...
                        if ( Environment.OSVersion.Platform.Equals( PlatformID.Win32NT ) )
                            if ( destinationPath[ ^1 ] == '/' ) destinationPath = destinationPath.Substring( 0, destinationPath.Length - 1 ) + "\\";
